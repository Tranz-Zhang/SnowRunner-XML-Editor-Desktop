...
namespace SnowPakTool {
	...
	public static class ZipPakHelper {
		...
		public static ushort ZipVersion => 0x14; // PKZIP 2.0, matching SnowRunner's current PAK layout.
		private static ushort ZipVersionMadeBy => 0x0314; // Unix origin + PKZIP 2.0.
		private static ushort FixedDosTime => 0x1800; // 1980-01-01 03:00:00.
		private static ushort FixedDosDate => 0x0021;
		private static uint UnixFileAttributes => 0x81B6_0000; // 0100666 shifted into ZIP external attrs.
		...
		private static void StoreFiles ( Stream zipStream , IReadOnlyList<KeyValuePair<string , string>> files ) {
			...
			for ( int i = 0; i < count; i++ ) {
				...
				using ( var file = File.OpenRead ( location ) ) {
					...
					var compressionMethod = methods[i] = CanBeCompressed ( i , relativeName ) ? CompressionMethod.Deflate : CompressionMethod.Store;
					MemoryStream compressedFile = null;

					if ( compressionMethod == CompressionMethod.Deflate ) {
						compressedFile = new MemoryStream ();
						using ( var deflate = new DeflateStream ( compressedFile , CompressionLevel.Optimal , true ) ) {
							file.CopyTo ( deflate );
						}
					...
					var compressedLength = compressedLengths[i] = compressedFile?.Length ?? file.Length;
					...
					var header = new LocalHeader {
						...
						Time = times[i] = FixedDosTime ,
						Date = dates[i] = FixedDosDate ,
					...
					if ( compressedFile == null ) {
						...
						compressedFile.WriteTo ( zipStream );
			...
			for ( int i = 0; i < count; i++ ) {
				...
				var header = new DirectoryHeader {
					...
					ExternalAttributes = UnixFileAttributes ,
